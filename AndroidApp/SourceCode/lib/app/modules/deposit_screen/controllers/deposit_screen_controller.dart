...
class DepositScreenController extends GetxController {
    ...
    await BaseClient.safeApiCall(
      ...
      onSuccess: (response) {
        ...
        paymentGatewayModel.gateways!.forEach((element) {
          if (element.name == "PayPal" || element.name == "Stripe" || element.name == "Flutterwave" || element.id!.toInt() > 15) {
  ...
  sendDeposit() async {
    String token = MySharedPref.getData('token') ?? '';
    var data = isManual.value ? dio.FormData.fromMap({
      'receipt_image': [
        await dio.MultipartFile.fromFile(imagePath.value, filename: '')
      ],
